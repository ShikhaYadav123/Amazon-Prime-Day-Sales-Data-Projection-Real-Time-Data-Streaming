...
#session = boto3.Session(profile_name='Shikha', region_name='us-east-2')

session = boto3.Session(region_name='us-east-2') # Change the region name according to the dynamodb table's region
dynamodb = session.resource('dynamodb') # from the boto3 session which AWS resource we want to access, in this case dynamodb
table = dynamodb.Table('ordertable') # give the name of dynamodb table, in this case 'ordertable'
